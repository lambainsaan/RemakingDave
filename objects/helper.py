...
class Helper:
    def predict_path_down(self, direction = None, up = False):
    ...
    def predict_path_up(self, direction = None):
        """ This method is used to detect collisions of object.
        """
        path, left, top = [], self.legs_rect.left, self.rect.top
        mod_y =  int(0.5 * (1/self.gravity_coeff) * (self.dy*self.dy))
        y_divides_x_in = float(mod_y)/self.dx
        if direction == 'right':
            for y in range(1, mod_y + 1):
                if y_divides_x_in <= 0:
