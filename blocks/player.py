...
import blocks.helper as helper
...
__credits__ = ["Roshan Rakesh", "Shubham Jain",
               "Rhitik Bhatt", "Shubham Sharma"]
__license__ = "MIT"
...
class Player(block.Block, helper.Helper):
    ...
    def __init__(self, cordinate_x, cordinate_y):
        ...
        """
        self.action = 'jump'
        # This is a cycle object which cycles over the walking acitons
        self.walking_actions = cycle(walking_action)
        ...
        self.image = pygame.Surface(
            (self.sprite_x, self.sprite_y), pygame.SRCALPHA, 32)
        # It transforms the orignal image that is cropped out to be of the size double that of orignal size
        self.image = pygame.transform.scale(
            self.image, (self.magnified_player_x, self.magnified_player_y))
        ...
        self.draw_rect = pygame.Rect(
            self.draw_rect.left, self.draw_rect.top, self.draw_rect.width - 10, self.draw_rect.height)
        self.shoot = False
        ...
        # Keeps track of the rect of the leg of player
        self.legs_rect = self.calc_legs_rect()
        # self.right_rect
        # self.up_rect
        #
        # Holds the future values that legs_rect will attain, useful for detecting collisions
    ...
    def move_right(self):
        ...
        self.left = False
        if not self.is_jump:
            if self.action not in walking_action:
                self.action = 'walk-1'
            self.action = next(self.walking_actions)
        self.draw_rect.right = binder.WIDTH if self.draw_rect.right + \
            self.dx > binder.WIDTH else self.draw_rect.right + self.dx
        if self.going_down:
    ...
    def move_left(self):
        ...
        if not self.is_jump:
            if self.action not in walking_action:
                self.action = 'walk-1'
            self.action = next(self.walking_actions)
        self.draw_rect.left = 0 if self.draw_rect.left - \
            self.dx < 0 else self.draw_rect.left - self.dx
        if self.going_down:
    ...
    def shoot_bullet(self):
        ...
        """
        self.bullet = Bullet.Bullet(self.draw_rect.left + self.magnified_player_x //
                                    2, self.draw_rect.top + self.magnified_player_y // 2, self.left)
    ...
    def on_ground(self):
        ...
        if self.is_jump:
            for i, ele in enumerate(self.future_leg_rects):
                colliding_objects = pygame.sprite.spritecollide(
                    ele, binder.bricks, False)
                if len(colliding_objects) != 0:
                    self.on_brick = (
                        colliding_objects[0].rect.left, colliding_objects[0].rect.right)
    ...
    def update_image(self):
        ...
        white = 255, 255, 255
        self.image = pygame.Surface(
            (self.player_width, self.player_height), pygame.SRCALPHA, 32)
        self.sprite_x, self.sprite_y = self.x_y_in_spritesheet()
        area_of_image = (self.sprite_x, self.sprite_y,
                         self.player_width, self.player_height)
        self.legs_rect = self.calc_legs_rect()
        self.image.blit(cowboy_sprite, (0, 0), area_of_image)
        self.image = pygame.transform.scale(
            self.image, (self.player_width * 2, self.player_height * 2))
